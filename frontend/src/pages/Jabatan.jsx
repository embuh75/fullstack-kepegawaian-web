...
import { getAllJabatan, createJabatan, updateJabatan, deleteJabatan } from "../services/jabatanService";
import Modal from "../components/ui/Modal";
import ConfirmDialog from "../components/ui/ConfirmDialog";
import Pagination from "../components/ui/Pagination";
import EmptyState from "../components/ui/EmptyState";
import PageHeader from "../components/ui/PageHeader";
import { IconSearch, IconPlus, IconEdit, IconTrash, IconBriefcase, IconLoader } from "../components/icons";
...
export default function Jabatan() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return all;
    return all.filter((j) => j.nama.toLowerCase().includes(q) || j.kode.toLowerCase().includes(q));
  }, [all, search]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (editing) {
        await updateJabatan(editing.id, payload);
      ...
      if (apiErrors && typeof apiErrors === "object") {
        ...
        toast.error(err.response?.data?.message || "Gagal menyimpan data jabatan.");
      }
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Gagal menghapus data jabatan.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        icon={IconBriefcase}
        title="Data Jabatan"
        description="Kelola daftar jabatan pegawai di sekolah."
        count={all.length}
        action={
          isAdmin && (
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={search ? "Tidak ada jabatan yang cocok dengan pencarian." : "Tambahkan jabatan pertama untuk mulai."}
          />
        ) : (
          <>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/60 text-xs uppercase tracking-wide text-slate-400">
                    <th className="px-5 py-3 font-semibold">Nama Jabatan</th>
                  ...
                  {paged.map((item) => (
                    <tr key={item.id} className="transition hover:bg-slate-50/70">
                      ...
                      <td className="px-5 py-3.5">
                        <span className="badge bg-brand-50 text-brand-700">{item.kode}</span>
                      </td>
                      {isAdmin && (
                        <td className="px-5 py-3.5">
      ...
      <Modal
        ...
        description={editing ? "Perbarui detail jabatan." : "Isi detail jabatan baru."}
        size="sm"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="field-label">Nama Jabatan</label>
            <input
              type="text"
              value={form.nama}
      ...
      <ConfirmDialog
        ...
        description={`Jabatan "${deleteTarget?.nama}" akan dihapus permanen. Pegawai dengan jabatan ini juga akan ikut terhapus (cascade).`}
        loading={deleting}
